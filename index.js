 ...
 */
server.post('/projects', projectExists, (req, res) => {
...
/* DELETE /projects/:id: A rota deve deletar o projeto 
 * com o id presente nos parâmetros da rota;
 */
server.delete('/projects/:id', projectExists, (req, res) => {
  const { id } = req.params;

  const projectToRemove = projects.findIndex(p => p.id == id);
  projects.slice(projectToRemove, 1);
